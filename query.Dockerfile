...
CMD ./jaeger-query --query.ui-config=ui.json --grpc-storage-plugin.binary=./query --grpc-storage-plugin.configuration-file=cfg/plugin-config.yaml --metrics-backend=none --log-level=debug
